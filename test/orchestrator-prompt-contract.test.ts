...
const normalizeTrailingNewline = (text: string) => text.replace(/\n$/, "");

const expectedPrompts = {
  "delegation.md": `# Subagent Delegation
...
Run independent assignments in parallel. Use background execution only when their results are not needed for your next step; keep doing useful work instead of polling.`,
  "orchestrator.md": `# Subagent Orchestrator
...
Report completion only when the integrated outcome satisfies the user's objective and appropriate verification is complete. If no defensible path remains, report the evidence gathered, attempted paths, blocker, and exact input needed to proceed.`,
  "orchestrator-reminder.md": `Orchestrator mode is active. Understand the user's objective and design an appropriate workflow before substantial direct work. Delegate suitable, non-overlapping ownership to the best-matching subagents; run independent assignments in parallel, synthesize their handoffs, and keep the overall outcome with the parent. Work directly when it is clearly better.

{{compactTypeList}}`,
};

describe("shipped orchestrator prompt contracts", () => {
  it("matches the accepted prompt bodies, allowing one trailing newline", () => {
    for (const [name, expected] of Object.entries(expectedPrompts)) {
      expect(normalizeTrailingNewline(readFileSync(promptPath(name), "utf-8")), name).toBe(expected);
    }
  });
});
...
function context(systemPrompt = "base prompt") {
  return {
    hasUI: false,
...
const parentOnlyBlock = (tag: string, content: string) => `<${tag}>\n${content}\n</${tag}>`;

describe("orchestrator prompt lifecycle", () => {
  let tmpDir: string;
  let agentDir: string;
  ...
  function register(orchestrator: boolean) {
    writeFileSync(join(tmpDir, ".pi", "subagents.json"), JSON.stringify({ orchestrator, schedulingEnabled: false }));
    const extension = makePi();
    subagentsExtension(extension.pi);
    return extension;
  }

  beforeEach(() => {
    tmpDir = mkdtempSync(join(tmpdir(), "pi-orchestrator-contract-"));
    agentDir = mkdtempSync(join(tmpdir(), "pi-orchestrator-contract-agentdir-"));
  ...
  it("injects only the selected parent prompt after stripping every parent-only block", () => {
    const original = [
      "base prompt",
      parentOnlyBlock("pi-subagents-delegation", "old delegation"),
      parentOnlyBlock("pi-subagents-orchestrator", "old orchestrator"),
      parentOnlyBlock("pi-subagents-orchestrator-reminder", "old reminder"),
    ].join("\n\n");

    const off = register(false);
    const offResult = off.handlers.get("before_agent_start")?.({ systemPrompt: original }, context());
    expect(offResult.systemPrompt).toContain("Use `Agent` when a matching subagent");
    expect(offResult.systemPrompt).not.toContain("Act as the parent orchestrator");
    expect(offResult.systemPrompt).not.toContain("old delegation");
    expect(offResult.systemPrompt).not.toContain("old orchestrator");
    expect(offResult.systemPrompt).not.toContain("old reminder");

    const on = register(true);
    const onResult = on.handlers.get("before_agent_start")?.({ systemPrompt: original }, context());
    expect(onResult.systemPrompt).toContain("Act as the parent orchestrator");
    expect(onResult.systemPrompt).not.toContain("Use `Agent` when a matching subagent");
  });

  it("never reinjects a parent block into a subagent system prompt", () => {
    const extension = register(true);
    const original = `${parentOnlyBlock("pi-subagents-orchestrator", "old orchestrator")}\n<active_agent name="general-purpose"/>`;
    const result = extension.handlers.get("before_agent_start")?.({ systemPrompt: original }, context());

    expect(result.systemPrompt).toContain('<active_agent name="general-purpose"/>');
    expect(result.systemPrompt).not.toContain("old orchestrator");
    expect(result.systemPrompt).not.toContain("Act as the parent orchestrator");
  });

  it("inserts one hidden reminder immediately before a real final user message", () => {
    const extension = register(true);
    const ctx = context();
    extension.handlers.get("input")?.({ source: "interactive" });
    const messages = [{ role: "assistant" }, { role: "user", content: "real input" }];

    const result = extension.handlers.get("context")?.({ messages }, ctx);
    ...
    expect(result.messages[1].content).toContain("Understand the user's objective");
    expect(result.messages[2]).toBe(messages[1]);
    expect(extension.handlers.get("context")?.({ messages }, ctx)).toBeUndefined();
  });

  it("skips reminders for extension or steering input, non-user finals, Off mode, and subagents", () => {
    const cases = [
      { orchestrator: true, input: { source: "extension" }, messages: [{ role: "user" }], prompt: "base prompt" },
      { orchestrator: true, input: { source: "interactive", streamingBehavior: "steer" }, messages: [{ role: "user" }], prompt: "base prompt" },
      { orchestrator: true, input: { source: "interactive" }, messages: [{ role: "assistant" }], prompt: "base prompt" },
      { orchestrator: false, input: { source: "interactive" }, messages: [{ role: "user" }], prompt: "base prompt" },
      { orchestrator: true, input: { source: "interactive" }, messages: [{ role: "user" }], prompt: '<active_agent name="Explore"/>' },
    ];

    for (const testCase of cases) {
      const extension = register(testCase.orchestrator);
      extension.handlers.get("input")?.(testCase.input);
      expect(extension.handlers.get("context")?.({ messages: testCase.messages }, context(testCase.prompt))).toBeUndefined();
    }
  });
});
