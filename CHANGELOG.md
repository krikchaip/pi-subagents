...
### Changed
- **FleetView focus and keyboard ownership.** FleetView now focuses only via `Shift+←` and closes via `Esc` or `Shift+←`; plain `↓`/`←` no longer activate it from the prompt. When active, FleetView owns the keyboard: `↑`/`↓` move and clamp selection, `Enter` opens the selected conversation, and unrelated keys are consumed as no-ops instead of leaking into the editor. When another menu/dialog owns focus, FleetView stays visible but consumes nothing. The active state now uses a left rail with a `FLEET LIVE` heading (`FLEET` themed as markdown heading/orange, `LIVE` as warning/yellow) so focus state is visible at a glance.
